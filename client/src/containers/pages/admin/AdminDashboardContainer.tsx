...
import {
  ...
  RecentActivityData,
  RecentActivityItem,
} from "@pages/admin/AdminDashboardPage/types";
import {
...
const AdminDashboardContainer = () => {
  ...
  const {
    ...
    error: activityError,
    data: activityData,
  } = useQuery<RecentActivityData>(GET_RECENT_ACTIVITY);
  const {
    loading: quickStatsLoading,
    error: quickStatsError,
    data: quickStatsData,
  } = useQuery<QuickStatsData>(GET_QUICK_STATS, {
  ...
  const completionRate =
    statsData?.completed_attempts && statsData?.total_attempts
      ? Math.round(
          (statsData.completed_attempts.aggregate.count /
            statsData.total_attempts.aggregate.count) *
            100,
        )
      : 0;
  const avgScore = statsData?.quiz_attempts_aggregate.aggregate.avg.score
    ? Math.round(statsData.quiz_attempts_aggregate.aggregate.avg.score)
    : 0;

  const avgDuration = React.useMemo(() => {
    if (
      !quickStatsData?.quiz_attempts_with_duration ||
      quickStatsData.quiz_attempts_with_duration.length === 0
    ) {
      return 0;
    }

    const durations = quickStatsData.quiz_attempts_with_duration.map(
      (attempt) => {
  ...
  const newQuizzesToday =
    quickStatsData?.new_quizzes_today.aggregate.count || 0;
