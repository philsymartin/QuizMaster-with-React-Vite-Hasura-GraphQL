...
export const INSERT_FEEDBACK_KEYWORD = gql`
  mutation InsertFeedbackKeyword($objects: [feedback_keywords_insert_input!]!) {
    insert_feedback_keywords(
      objects: $objects
      on_conflict: { constraint: feedback_keywords_pkey, update_columns: [] }
    ) {
      affected_rows
      returning {
        keyword_id
...
export const INSERT_FEEDBACK_KEYWORD_MAPPING = gql`
  mutation InsertFeedbackKeywordMapping(
    $objects: [feedback_keyword_mapping_insert_input!]!
  ) {
    insert_feedback_keyword_mapping(
      objects: $objects
      on_conflict: {
        constraint: feedback_keyword_mapping_pkey
        update_columns: []
      }
    ) {
      affected_rows
...
export const UPDATE_FEEDBACK_KEYWORD_EXTRACTED = gql`
  mutation UpdateFeedbackKeywordExtracted(
    $feedback_ids: [Int!]
