...
export const ADD_QUESTION_AND_OPTIONS = gql`
  mutation AddQuestionAndOptions(
    ...
    options: insert_options(
      objects: $options
      on_conflict: { constraint: options_option_text_key, update_columns: [] }
    ) {
      returning {
        option_id
        option_text
...
export const LINK_QUESTION_OPTIONS = gql`
  mutation LinkQuestionOptions($objects: [question_options_insert_input!]!) {
...
export const DELETE_QUESTION_OPTIONS = gql`
  mutation DeleteQuestionOptions($question_id: Int!) {
    delete_question_options(where: { question_id: { _eq: $question_id } }) {
      affected_rows
    }
  }
`;
...
export const UPDATE_QUESTION = gql`
  mutation UpdateQuestion(
    ...
    update_questions_by_pk(
      ...
      _set: { question_text: $question_text, question_type: $question_type }
    ) {
      question_id
      question_text
      question_type
...
export const START_QUIZ_ATTEMPT = gql`
  mutation StartQuizAttempt($quiz_id: Int!, $user_id: Int!) {
    insert_quiz_attempts_one(object: { quiz_id: $quiz_id, user_id: $user_id }) {
      attempt_id
      start_time
    }
  }
...
export const SUBMIT_QUIZ_ATTEMPT = gql`
  mutation SubmitQuizAttempt(
    ...
    update_quiz_attempts_by_pk(
      ...
      _set: { end_time: $end_time, score: $score, user_id: $user_id }
    ) {
      attempt_id
      score
      end_time
