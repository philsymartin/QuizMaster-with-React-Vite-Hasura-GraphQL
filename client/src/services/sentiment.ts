...
export const analyzeSentiment = async (
  ...
  while (attempt <= maxRetries) {
    try {
      ...
      if (
        response.status === 503 ||
        data.message?.includes("model is loading")
      ) {
        attempt++;
...
export const analyzeKeywords = async (
  ...
  while (attempt <= maxRetries) {
    try {
      if (attempt > 0) {
        console.log(
          `Retry attempt ${attempt} of ${maxRetries} for keyword analysis`,
        );
        await delay(delayMs);
      }
      ...
      if (
        response.status === 503 ||
        data.error?.includes("model is currently loading")
      ) {
        attempt++;
        if (attempt > maxRetries) {
  ...
  throw new KeywordAnalysisError(
    "Maximum retries exceeded for keyword analysis",
  );
};
