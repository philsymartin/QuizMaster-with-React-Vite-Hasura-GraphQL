...
function* startQuizSaga(
  action: ReturnType<typeof startQuizAttempt>,
  ...
  try {
    ...
    if (
      quizAttemptState.isInitialized &&
      quizAttemptState.currentQuiz?.quizId === action.payload.quizId
    ) {
      console.log("Quiz already fully initialized, returning");
      return;
    }
    const user: { user_id: number } | null = yield select(
    ...
    const checkExistingAttempt: ApolloQueryResult<any> = yield call(
      [client, client.query],
      {
    ...
    if (checkExistingAttempt.data?.quiz_attempts?.length > 0) {
      ...
      const startAttemptResult: StartQuizAttemptResponse = yield call(
      ...
      console.log(
        "returned start_time",
        startAttemptResult.data.insert_quiz_attempts_one.start_time,
      );
    ...
    const questionsResult: ApolloQueryResult<any> = yield call(
      [client, client.query],
      {
    ...
    yield take([
      resetQuizAttempt.type,
      submitQuizSuccess.type,
      submitQuizFailure.type,
    ]);
...
function* submitQuizSaga(): Generator<any, void, any> {
  try {
    ...
    const correctAnswers = state.questions.reduce(
      (count: number, question: QuizQuestion) => {
    ...
    const answersInput = Object.entries(state.answers).map(
      ([questionId, optionId]) => ({
