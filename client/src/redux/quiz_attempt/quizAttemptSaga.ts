// import { call, put, select, takeLatest, delay, take, fork, cancel, race, cancelled } from 'redux-saga/effects';
...
//     }>;
// }
// // Timer task reference for cancellation across sagas
// let globalTimerTask: Task | null = null;

// function* startTimerSaga(): SagaIterator {
//     try {
//         while (true) {
...
//             yield put(startQuizAttemptError('User not authenticated'));
//             return;
//         }
...
//         // Start timer and store the task reference globally
//         globalTimerTask = yield fork(startTimerSaga);

//         // Wait for quiz completion or reset
//         const { resetAction } = yield race({
//             completion: take([submitQuizSuccess.type, submitQuizFailure.type]),
//             resetAction: take([resetQuizAttempt.type, endQuizEarly.type])
//         });
//         if (globalTimerTask) {
//             yield cancel(globalTimerTask);
//             globalTimerTask = null;
//         }
//         // If quiz was reset or ended early, perform cleanup
//         if (resetAction) {
//             yield call(cleanupSaga);
//         }
//     } catch (error) {
...
//     } finally {
//         if (yield cancelled()) {
//             console.log('Quiz start saga cancelled');
//             if (globalTimerTask) {
//                 yield cancel(globalTimerTask);
//                 globalTimerTask = null;
//             }
...
//         if (state.isComplete) {
//             console.log('Quiz already complete, not submitting again');
//             return;
//         }
//         const user: { user_id: number } | null = yield select((state: RootState) => state.auth.user);
//         if (!user || !user.user_id) {
...
//     const state: ReturnType<typeof selectQuizAttemptState> = yield select(selectQuizAttemptState);
//     const user: { user_id: number } | null = yield select((state: RootState) => state.auth.user);
//     // Cancel any existing timer
//     if (globalTimerTask) {
//         yield cancel(globalTimerTask);
//         globalTimerTask = null;
//     }
//     // Only cleanup if there's an active attempt that hasn't been completed
//     if (state.attemptId && !state.isComplete && user?.user_id) {
//         try {
//             yield call([client, client.mutate], {
//                 mutation: gql`
//                     mutation UpdateQuizAttempt($attempt_id: Int!, $end_time: timestamptz!) {
...
//                 },
//             });
//             console.log('Quiz attempt cleaned up successfully');
//         } catch (error) {
//             console.error('Error cleaning up quiz attempt:', error);
//         }
//     }
// }
// export function* watchQuizAttemptSaga(): SagaIterator {
//     yield takeLatest(startQuizAttempt.type, startQuizSaga);
//     yield takeLatest(submitQuizRequest.type, submitQuizSaga);
//     yield takeLatest(endQuizEarly.type, cleanupSaga);
