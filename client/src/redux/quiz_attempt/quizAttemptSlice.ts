...
//             totalQuestions: number;
//         }>) => {
//             // Only initialize if not already initialized or if quiz ID changes
//             if (!state.isInitialized || state.currentQuiz?.quizId !== action.payload.quizId) {
...
//             if (!state.isComplete && !state.isSubmitting) {
