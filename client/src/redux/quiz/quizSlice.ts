...
const quizSlice = createSlice({
  ...
  reducers: {
    ...
    updateQuizSettingsSuccess: (state, action: PayloadAction<Quiz>) => {
      ...
      if (
        state.selectedQuiz &&
        state.selectedQuiz.quiz_id === action.payload.quiz_id
      ) {
        state.selectedQuiz = action.payload;
      }
    ...
    addQuizRequest: (
      state,
      action: PayloadAction<
        Omit<Quiz, "quiz_id" | "created_at" | "updated_at">
      >,
    ) => {
      state.loading = true;
      state.error = null;
    ...
    deleteQuizSuccess: (state, action: PayloadAction<number>) => {
      ...
      state.quizzes = state.quizzes.filter(
        (quiz) => quiz.quiz_id !== action.payload,
      );
...
export const selectSelectedQuiz = (state: { quiz: QuizState }) =>
...
export const selectAddQuestionModalOpen = (state: { quiz: QuizState }) =>
  state.quiz.addQuestionModalOpen;
export const selectAddQuestionLoading = (state: { quiz: QuizState }) =>
  state.quiz.addQuestionLoading;
export const selectAddQuestionError = (state: { quiz: QuizState }) =>
  state.quiz.addQuestionError;
export const selectDeleteQuestionLoading = (state: { quiz: QuizState }) =>
  state.quiz.deleteQuestionLoading;
export const selectDeleteQuestionError = (state: { quiz: QuizState }) =>
  state.quiz.deleteQuestionError;
export const selectEditingQuestion = (state: { quiz: QuizState }) =>
  state.quiz.editingQuestion;
export const selectIsEditing = (state: { quiz: QuizState }) =>
  state.quiz.isEditing;
export const selectUpdateSettingsLoading = (state: { quiz: QuizState }) =>
  state.quiz.updateSettingsLoading;
export const selectUpdateSettingsError = (state: { quiz: QuizState }) =>
  state.quiz.updateSettingsError;
