...
import {
  ...
  GET_QUIZZES_BASIC,
  GET_QUIZZES_WITH_TOPICS,
  GET_QUIZ_QUESTIONS,
  GET_SINGLE_QUESTION,
} from "../../api/queries/quizzes";
import {
  ADD_QUESTION_AND_OPTIONS,
  LINK_QUESTION_OPTIONS,
  DELETE_QUESTION,
  DELETE_QUESTION_OPTIONS,
  DELETE_OPTION,
  UPDATE_QUESTION,
  INSERT_NEW_OPTIONS,
} from "../../api/mutations/questionsMutate";
import {
  CREATE_QUIZ,
  DELETE_QUIZ,
  UPDATE_QUIZ_SETTINGS,
} from "@mutations/quizMutate";
...
function* fetchQuizzesSaga() {
  try {
    const response: ApolloQueryResult<QuizzesQueryResponse> = yield call(
      [client, client.query],
      {
...
function* updateQuizSettingsSaga(
  action: ReturnType<typeof updateQuizSettings>,
) {
  try {
...
function* addQuestionSaga(action: ReturnType<typeof addQuestionRequest>) {
  try {
    ...
    const addQuestionResult: { data: AddQuestionResponse } = yield call(
    ...
    const getOptionsResult: { data: GetOptionsResponse } = yield call(
      [client, client.query],
      {
        query: GET_OPTIONS,
        variables: {
    ...
    const optionsMap = new Map(
      getOptionsResult.data.options.map((opt) => [
        opt.option_text,
        opt.option_id,
      ]),
    );
    ...
    const completeQuestionResult: ApolloQueryResult<SingleQuestionResponse> =
...
function* deleteQuestionSaga(action: ReturnType<typeof deleteQuestionRequest>) {
  try {
    ...
    const optionsResult: ApolloQueryResult<{
      question_options: { option_id: number }[];
    }> = yield call([client, client.query], {
      query: GET_QUESTION_OPTIONS,
      variables: { question_id: questionId },
    });

    const optionIds = optionsResult.data.question_options.map(
      (qo) => qo.option_id,
    );
    ...
    if (quiz) {
      const questionsResponse: ApolloQueryResult<QuestionsQueryResponse> =
        yield call([client, client.query], {
          query: GET_QUIZ_QUESTIONS,
...
function* editQuestionSaga(action: ReturnType<typeof editQuestionRequest>) {
  try {
    const { question_id, question_text, question_type, options } =
      action.payload;
    ...
    const existingQuestion: ApolloQueryResult<SingleQuestionResponse> =
      yield call([client, client.query], {
        query: GET_SINGLE_QUESTION,
      ...
      .filter(
        (opt) =>
          !existingOptions.some(
            (eo) => eo.option.option_text === opt.option_text,
          ),
      )
      .map((opt) => ({
    ...
    const optionsMap = new Map(
      allOptionsResult.data.options.map((opt) => [
        opt.option_text,
        opt.option_id,
      ]),
    );
    ...
    const updatedQuestion: ApolloQueryResult<SingleQuestionResponse> =
      yield call([client, client.query], {
        query: GET_SINGLE_QUESTION,
        variables: { question_id },
        fetchPolicy: "network-only",
...
function* addQuizSaga(action: ReturnType<typeof addQuizRequest>) {
  try {
    const { title, description, difficulty, time_limit_minutes } =
      action.payload;
